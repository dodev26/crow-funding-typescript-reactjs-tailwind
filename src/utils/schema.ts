...
export const schema = yup.object().shape({
  ...
  idDoc: yup.string().required('ID document is required')
})
...
export const campaignSchema = yup.object().shape({
  ...
  raised_amount: yup
    ...
    .positive('Raised amount  must be positive'),
  amount_prefilled: yup
    .number()
    .transform((value) => (isNaN(value) ? undefined : value))
    .nullable()
    .required('Amount  prefilled is required')
    .positive('Amount  prefilled must be positive'),
