...
export type PersonalFormFields = Required<
  Pick<SchemaType, 'firstName' | 'lastName' | 'dateOfBirth' | 'phone' | 'avatarURL'> & {
    idDoc: string
  }
>
...
const PersonalSchema = schema.pick(['firstName', 'lastName', 'dateOfBirth', 'phone', 'avatarURL', 'idDoc'])
...
export const ProfilePage = () => {
  ...
  const {
    ...
    setValue: setPersonalValueForField,
    reset: resetPersonalField,
    watch: watchPersonalField,
    formState: { errors: errorPersonalField, isValid: isValidPersonalField }
  ...
  const {
    ...
    control: controlCredentialsField,
    reset: resetCredentialsField,
    formState: { errors: errorCredentialsField },
    setError
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      const queries = query(colUsersRef, where('uid', '==', UserData?.uid))
      ...
      if (data && data.length === 1) {
        const personalData: PersonalFormFields = {
          ...omit(data[0], ['createdAt', 'updatedAt', 'email', 'password']),
          dateOfBirth: data[0].dateOfBirth ? (data[0].dateOfBirth as unknown as Timestamp).toDate() : new Date()
        } as PersonalFormFields
  ...
  const handleUpdateCredentialsInfo = HandleSubmitCredentials(
    async (data: Required<Omit<CredentialsFormFields, 'email'>>) => {
      setIsLoading(true)
      const dataUpdatePassword = omit(data, ['email', 'confirmPassword'])
      try {
        const credential = EmailAuthProvider.credential(
          String(UserData?.email),
          String(dataUpdatePassword.currentPassword)
        )
    ...
    <>
      ...
      <div className='w-full  max-w-[1098px] px-5 md:px-10 md:py-8 py-7 mx-auto lg:px-[130px] lg:pt-[50px] pb-[60px] bg-inherit dark:bg-darkSecondary dark:rounded-xl'>
        ...
        <form onSubmit={handleUpdatePersonalInfo} className='mt-[55px] w-full'>
          <div className='flex items-center justify-between'>
            ...
            {updatePersonalInfo ? (
              <button
                ...
                className='flex items-center text-error justify-center'
              >
                <svg
              ...
              <button
                ...
                type='button'
                onClick={handleTogglePersonal}
                className='flex items-start gap-x-2 text-secondary text-xl font-semibold'
              >
                <svg
                  xmlns='http://www.w3.org/2000/svg'
                  fill='none'
            ...
            <Controller
              ...
              render={({ field: { value, onChange, name, ...rest } }) => (
                <UploadAvatar
                  ...
                  open={updatePersonalInfo}
                  avatarURL={(previewImage || user?.photoURL || value) as string}
                  name={name}
                  onChange={(e) => {
          ...
          <FormRow className='gap-x-[41px] mt-[35px] grid-cols-1 md:grid-cols-2'>
            <FormGroup>
              <label htmlFor='firstName' className='text-text3 text-sm'>
                First Name
              </label>
              <Input
                disabled={!updatePersonalInfo}
                type='text'
                errorField={errorPersonalField.firstName?.message}
                control={controlPersonalField}
                name='firstName'
                id='firstName'
            ...
            <FormGroup>
              <label htmlFor='lastName' className='text-text3 text-sm'>
                Last Name
              </label>
              <Input
                disabled={!updatePersonalInfo}
                type='text'
                errorField={errorPersonalField.lastName?.message}
                control={controlPersonalField}
                name='lastName'
                id='lastName'
          ...
          <FormRow className='gap-x-[41px] mt-[5px] grid-cols-1 md:grid-cols-2'>
            <FormGroup>
              <label htmlFor='firstName' className='text-text3 text-sm'>
                Date of Birth
              </label>
              <Controller
                ...
                render={({ field: { value, onChange, ...props } }) => (
                  <DatePicker
                    disabled={!updatePersonalInfo}
                    format='dd/MM/yyyy'
                    errorField={errorPersonalField.dateOfBirth?.message}
                    value={value}
            ...
            <FormGroup>
              <label htmlFor='phone' className='text-text3 text-sm'>
                Mobile Number
              </label>
              <InputNumber
                disabled={!updatePersonalInfo}
                control={controlPersonalField}
                errorField={errorPersonalField.phone?.message}
                name='phone'
                id='phone'
          ...
          {updatePersonalInfo && (
            <Button
              disabled={!isValidPersonalField || isLoading}
              isLoading={isLoading}
              type='submit'
              kind='primary'
        ...
        <form className='mt-[55px] w-full'>
          ...
          <FormRow className='gap-x-[41px] mt-[35px] grid-cols-1 md:grid-cols-2'>
            <FormGroup>
              ...
              <Input
                ...
                errorField={errorCredentialsField.email?.message}
                type='email'
                control={controlCredentialsField}
                name='email'
                id='email'
          ...
          <FormRow className='gap-x-[41px] mt-[35px] grid-cols-1 md:grid-cols-2'>
            <FormGroup>
              ...
              <Input
                ...
                errorField={errorCredentialsField.currentPassword?.message}
                type='password'
                control={controlCredentialsField}
          ...
          <FormRow className='gap-x-[41px] mt-[5px] grid-cols-1 md:grid-cols-2'>
            <FormGroup>
              <label htmlFor='password' className='text-text3 text-sm'>
                New Password
              </label>
              <Input
                disabled={!updateCredentials}
                type='password'
                control={controlCredentialsField}
                errorField={errorCredentialsField.password?.message}
                name='password'
                placeholder='Your new password'
              />
            </FormGroup>
            <FormGroup>
              <label htmlFor='confirmPassword' className='text-text3 text-sm'>
                Confirm Password
              </label>
              <Input
                disabled={!updateCredentials}
                type='password'
                control={controlCredentialsField}
                errorField={errorCredentialsField.confirmPassword?.message}
          ...
          {updateCredentials && (
            <Button
              onClick={handleUpdateCredentialsInfo}
              disabled={isLoading}
              isLoading={isLoading}
              kind='primary'
              className='w-full  max-w-[164px]'
