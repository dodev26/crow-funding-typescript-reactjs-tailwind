...
export const SignInPage = () => {
  ...
  return (
    <>
      <h1 className='mb-1 text-lg capitalize lg:text-xl  text-center lg:mb-3  font-semibold text-text1 dark:text-white'>{t(lang.heading())}
      </h1>
      <p className='text-xs mb-5  text-text3 font-normal text-center lg:text-sm'>{t(lang.dontHaveAccount())} <Link to="/sign-up" className='text-primary font-medium underline'>{t(lang.signUp())}</Link></p>
      <ButtonGoogle text={t(lang.signInWithGG())} />
      ...
      <form onSubmit={onSubmit}>
        <FormGroup>
          ...
          <Input disabled={loading} errorField={errors.email?.message} id="email" name='email' type="text" control={control} placeholder={t(lang.pEmail())} />
        </FormGroup>
        <FormGroup className='mb-0'>
          <Label htmlFor='password' className="dark:text-text3  text-sm">{t(lang.password())}</Label>
          <Input disabled={loading} errorField={errors.password?.message} id="password" name='password' type="password" control={control} placeholder={t(lang.pPassword())} />
        </FormGroup>
        <div className='flex items-center justify-end mb-5'>
          <Link to="/forgot-password" className='text-primary text-sm font-medium hover:underline'>{t(lang.forgotPassword())}</Link>
        </div>
